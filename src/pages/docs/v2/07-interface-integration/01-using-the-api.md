...
In this guide we will create a web interface that consumes and displays data from the Valueswap Subgraph. The goal is to provide a quick overview of a setup that you can extend to create your own UIs and analytics around Valueswap data.
...
```javascript
yarn create react-app valueswap-demo
cd valueswap-demo
...
We need to set up some middleware in order to make requests to the Valueswap subgraph and receive data. To do this we'll use Apollo and create a graphql client to handle this.

1. Add the imports shown below and instantiate a new client instance. Notice how we use the link to the Valueswap subgraph here.
...
export const client = new ApolloClient({
 link: new HttpLink({
   uri: "https://thegraph.com/explorer/subgraph/valueswap/valueswap-v2"
 }),
...
Next we'll construct our query and fetch data. For this example we will fetch some data about the Dai token on Valueswap V2. We'll get the current price, and total liquidity across all pairs. We'll be using the Dai address as an id in this query. We'll also fetch the USD price of ETH to help create USD conversion for Dai data.
...
Now we're ready to use these queries to fetch data from the Valueswap V2 subgraph. To do this we can use the `useQuery` hook which uses our client instance to fetch data, and gives us live info about the status of the request. To do this add the following to your `App.js` file:
