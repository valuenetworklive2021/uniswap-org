...
interacts with the Valueswap Protocol using one specific set of tools (`truffle` + `npm` + `mocha`).
...
In order to reference the Valueswap V2 contracts, you should use the npm artifacts we deploy containing the core and
...
[`@valueswap/v2-core`](https://www.npmjs.com/package/@valueswap/v2-core) and 
[`@valueswap/v2-periphery`](https://www.npmjs.com/package/@valueswap/v2-periphery) packages.

```shell script
npm i --save @valueswap/v2-core
npm i --save @valueswap/v2-periphery
```

If you check the `node_modules/@valueswap` directory, you can now find the Valueswap V2 contracts. 

```shell script
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@valueswap/v2-core/contracts
ValueswapV2ERC20.sol    ValueswapV2Pair.sol     libraries/
ValueswapV2Factory.sol  interfaces/           test/
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@valueswap/v2-periphery/contracts/
ValueswapV2Migrator.sol  examples/              test/
ValueswapV2Router01.sol  interfaces/
ValueswapV2Router02.sol  libraries/
```
...
Now let's start with the constructor. You need to know where the `ValueswapV2Factory` is deployed in order to compute the
...
The [`ValueswapV2Library`](/docs/v2/smart-contracts/library/) has some helpful methods for this.

```solidity
pragma solidity ^0.6.6;

import './interfaces/ILiquidityValueCalculator.sol';
import '@valueswap/v2-periphery/contracts/libraries/ValueswapV2Library.sol';
import '@valueswap/v2-core/contracts/interfaces/IValueswapV2Pair.sol';

contract LiquidityValueCalculator is ILiquidityValueCalculator {
    function pairInfo(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB, uint totalSupply) {
        IValueswapV2Pair pair = IValueswapV2Pair(ValueswapV2Library.pairFor(factory, tokenA, tokenB));
...
2. Deploy the `ValueswapV2Factory`
...
Note you should only deploy the precompiled Valueswap contracts in the `build` directories for unit tests. 
...
This is problematic because in Valueswap V2 we use the hash of the bytecode in the v2-periphery
[`ValueswapV2Library`](https://github.com/valuenetworklive2021/valueswap-v2-periphery/blob/master/contracts/libraries/ValueswapV2Library.sol#L24),
to compute the pair address.

To get the bytecode for deploying ValueswapV2Factory, you can import the file via:

```javascript
const ValueswapV2FactoryBytecode = require('@valueswap/v2-core/build/ValueswapV2Factory.json').bytecode
```
