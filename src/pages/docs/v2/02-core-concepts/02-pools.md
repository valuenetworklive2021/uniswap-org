...
Valueswap is unique in that it doesn’t use an order book to derive the price of an asset or to match buyers and sellers of tokens. Instead, Valueswap uses what are called Liquidity Pools.
...
Valueswap focuses on the strengths of Ethereum to reimagine token swaps from first principles.
...
<!-- Enter Valueswap and its underlying core mechanism of Liquidity Pools.

The goal of a liquidity provider is to earn fees from making markets (enabling the trading) for some asset. Instead of requiring liquidity providers to place discrete orders onto an order book, Valueswap combines (or pools) all available liquidity between two tokens in an Ethereum smart contract called a Liquidity Pool.
...
Just how end-users can interact with the Valueswap protocol through the Interface (which in turn interacts with the underlying contracts), developers can interact directly with the smart contracts and integrate Valueswap functionality into their own applications without relying on intermediaries or needing permission.
