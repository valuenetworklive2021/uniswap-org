...
Entities define the schema of the subgraph, and represent the data that can be queried. Within each entity are sets of fields that store useful information related to the entity. Below is a list of the available entities within the Valueswap Subgraph, and descriptions for the available fields.

To see an interactive sandbox of all entities see the [Graph Explorer](https://thegraph.com/explorer/subgraph/valueswap/valueswap-v2).
...
The Valueswap Factory entity is responsible for storing aggregate information across all Valueswap pairs. It can be used to view stats about total liquidity, volume, amount of pairs and more. There is only one ValueswapFactory entity in the subgraph.

| Field Name        | Value Type | Description                                                     |
| ----------------- | ---------- | --------------------------------------------------------------- |
| id                | ID         | factory address                                                 |
| pairCount         | Int        | amount of pairs created by the Valueswap factory                  |
...
| factory              | ValueswapFactory      | reference to Valueswap factory entity                                                                                 |
...
A user entity is created for any address that provides liquidity to a pool on Valueswap. This entity
...
Transaction entities are created for each Ethereum transaction that contains an interaction within Valueswap contracts. This subgraph tracks Mint, Burn, and Swap events on the Valueswap core contracts. Each transaction contains 3 arrays, and at least one of these arrays has a length of 1.
...
Mint entities are created for every emitted Mint event on the Valueswap core contracts. The Mint entity stores key data about the event like token amounts, who sent the transaction, who received the liquidity, and more. This entity can be used to track liquidity provisions on pairs.
...
Burn entities are created for every emitted Burn event on the Valueswap core contracts. The Burn entity stores key data about the event like token amounts, who burned LP tokens, who received tokens, and more. This entity can be used to track liquidity removals on pairs.
...
| mostLiquidTokens  | [TokenDayData!] | tokens with most liquidity in Valueswap                                            |
