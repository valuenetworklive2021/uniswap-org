...
Valueswap liquidity pools are autonomous and use the Constant Product Market Maker \(`x * y = k`\). This model was formalized and the smart contract implementation passed a lightweight formal verification.
...
Once an exchange is created the address can be retrieved with [`getExchange`](../connect-to-valueswap/#get-exchange-address).
...
Anyone who wants can join a Valueswap liquidity pool by calling the `addLiquidity` function.
